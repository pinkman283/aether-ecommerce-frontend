...
export const DepthText: React.FC<DepthTextProps> = ({
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      if (animateOnHover) {
        if (isHovered && autoOrbit && !activePointer) {
          const elapsed = (now - startTime) / 1000;
          const orbit = elapsed * safeOrbitSpeed * Math.PI * 2;
          target.x = baseRotation.x + Math.sin(orbit) * safeTilt * 0.4;
          target.y = baseRotation.y + Math.cos(orbit * 0.85) * safeTilt * 0.4;
        }
      } else {
  ...
  }, [animateOnHover, autoOrbit, baseRotation, pointerTracking, safeOrbitSpeed, safeSmoothing, safeTilt]);
